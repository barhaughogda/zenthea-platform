...
function WebsiteBuilderPageContent(props: WebsiteBuilderProps) {
  const {
    searchParams,
  ...
  const currentBlocks = useMemo(() => localBlocks || (websiteData?.websiteBuilder?.blocks as BlockInstance[]) || [], [localBlocks, websiteData]);
  ...
  const currentPages = useMemo(() => currentSiteStructure === 'multi-page' 
    ? (localPages || (websiteData?.websiteBuilder?.pages as PageConfig[]) || getDefaultPages())
    : [], [currentSiteStructure, localPages, websiteData]);
  
  const currentPage = useMemo(() => currentPages.find(p => p.id === currentPageId) || null, [currentPages, currentPageId]);
  const activePageBlocks = useMemo(() => currentPageId === 'home' ? currentBlocks : (currentPage?.blocks ?? []), [currentPageId, currentBlocks, currentPage]);
  
  const selectedBlock = useMemo(() => activePageBlocks?.find((b) => b.id === selectedBlockId) || null, [activePageBlocks, selectedBlockId]);
...
export default function WebsiteBuilder(props: WebsiteBuilderProps) {
  return (
